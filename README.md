...
- **80053 센서 상세 페이지**: psi/m 토글 버튼 (m: 백엔드 계산값, psi: ÷0.703 변환), depth_label 1번 기준 각주 표시, 초기값 첫 측정값 기준 각주 표시 (일별 모드)
- **PDF 출력**: NanumGothic.ttf 폰트 필요 (public 폴더), 관리자(현장 담당자 이름 + 권한 자동 표시), 설치현황(설치일자 자동 표시), 설치위치(센서 편집에서 입력한 값 자동 표시)
