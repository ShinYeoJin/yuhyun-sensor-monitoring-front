...
### 80053 수위계 전용 기능
- depth_label 1번/2번/3번 선택 버튼
- Poly(Polynomial 메인)/Linear(서브) 계산식 토글
- Polynomial: `P(psi) = A×R² + B×R + C` × 0.703
  - depth_label 1번: A=7.080E-08, B=-0.01296, C=106.0458
  - depth_label 2,3번: A=1.429E-07, B=-0.01532, C=118.4773
- Linear: `P(psi) = G × (초기값 - 현재값)` × 0.703
...
- 헤더 정보 (관리자 이름+권한 포함)
- 차트 (1차 관리기준선 포함, Y축 자동 범위)
- 범례 가운데 정렬
- 데이터 표 (엑셀과 동일한 소스로 수치 통일)
...
- 현장 관리 (현장 추가/편집/삭제, 센서 배정, 담당자 관리)
- 사용자 관리 (추가/수정/삭제/비활성화, 핸드폰번호/권한 관리)
- 파일 관리 (업로드/다운로드/삭제)
- QR 코드 (로그인 없이 센서 현재 상태 조회)
- 권한별 접근 제어
