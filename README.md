...
- **센서 관리**: 모니터링 / 센서 정의 / 계산식 관리 3개 탭, 상태 필터, 상세 페이지, QR 코드, 계산식 관리 탭: 관리자만 추가/수정/삭제 가능
- **새 센서 반자동 등록**: 에이전트가 새 센서 파일 감지 시 자동 DB 등록 → 관리자가 편집 UI에서 정보 수정
- **센서 상세**: 시간별 트렌드 차트, 측정 데이터 테이블, 날짜 범위 선택, 보고서 출력
- **계산식 관리**: 관리자가 웹에서 계산식 추가/수정/삭제 가능
  센서 편집 시 DB에서 동적으로 계산식 목록 로드
- **80053 수위계**: psi/m 단위 토글, depth_label 각주, 초기값 각주
- **비밀번호 변경**: 사이드바 하단에서 본인 비밀번호 변경 가능
- **본인 계정 권한 변경 방지**: 사용자 관리 페이지에서 
  본인 계정 행에는 수정/비활성화/삭제 버튼 표시 안 됨
- **PDF 출력**: 관리자 이름+권한, 설치현황, 설치위치 자동 표시
  NanumGothic 한글 폰트 적용
- **보고서 출력 모달**: 미리보기 PDF 표 형태로 표시
...
## 권한 구조 테이블 수정:
현재 admin role → Administrator로 DB 업데이트 완료
