      ...
      <div className="w-full max-w-md rounded-2xl bg-surface-card p-6 shadow-2xl">
        ...
        <div className="max-h-64 overflow-y-auto space-y-1 border border-line rounded-lg p-2">
          {available.length === 0 ? <p className="px-3 py-4 text-center font-mono text-[11px] text-ink-muted">추가 가능한 센서가 없습니다.</p>
            : available.map((s: any) => {
              const isAlready = currentIds.includes(s.id)
              return (
                <label key={s.id} className={['flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors', isAlready ? 'opacity-50 cursor-default' : 'hover:bg-surface-subtle'].join(' ')}>
                  <input type="checkbox" checked={selected.includes(s.id) || isAlready} disabled={isAlready}
                    onChange={() => !isAlready && setSelected(prev => prev.includes(s.id) ? prev.filter(i => i !== s.id) : [...prev, s.id])} className="rounded border-line" />
                  <div><p className="text-sm font-medium text-ink">{s.name}</p><p className="font-mono text-[10px] text-ink-muted">{s.sensor_type || '—'} {isAlready ? '(이미 등록됨)' : ''}</p></div>
                </label>
              )
            })}
        </div>
        <div className="flex gap-2 mt-4">
          ...
          <button onClick={() => onSave(selected)} disabled={selected.length === 0} className="flex-1 rounded-lg bg-brand py-2 font-mono text-sm text-white disabled:opacity-40 hover:bg-brand/90">추가</button>
        </div>
      </div>
    </div>
  )
...
function RemoveSensorModal({ siteSensors, onClose, onRemove }: {
  siteSensors: any[]; onClose: () => void; onRemove: (id: number) => void
}) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl bg-surface-card p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-ink">센서 삭제</h2>
          <button onClick={onClose} className="text-ink-muted hover:text-ink text-lg">×</button>
        </div>
        ...
        <div className="max-h-64 overflow-y-auto space-y-1 border border-line rounded-lg p-2">
          {siteSensors.length === 0 ? <p className="px-3 py-4 text-center font-mono text-[11px] text-ink-muted">등록된 센서가 없습니다.</p>
            : siteSensors.map((s: any) => (
              <div key={s.id} className="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-surface-subtle">
                <div><p className="text-sm font-medium text-ink">{s.name}</p><p className="font-mono text-[10px] text-ink-muted">{s.sensor_type || '—'}</p></div>
                <button onClick={() => onRemove(s.id)} className="rounded-md border border-red-200 px-2.5 py-1 font-mono text-[10px] text-red-400 hover:bg-red-50">제거</button>
              </div>
            ))}
        </div>
...
export default function SiteDetailPage() {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      if (isResizingLeft.current) { e.preventDefault(); document.body.style.userSelect = 'none'; document.body.style.cursor = 'col-resize'; setLeftWidth(Math.max(160, Math.min(360, e.clientX))) }
    }
  ...
  const saveIconPositions = useCallback(async (nextIcons: typeof icons) => {
    ...
    await fetch(`${API_BASE}/api/sites/${site.id}/sensor-positions`, {
      method: 'PATCH',
  ...
  useEffect(() => {
    ...
    Promise.all([siteApi.getAll(), sensorApi.getAll()])
      .then(([sitesData, sensorsData]) => {
        ...
        if (found.sensor_positions) {
          setIcons(Object.entries(found.sensor_positions as Record<string, any>).map(([key, val]: any) => ({ key, label: val.label || key, x: val.x, y: val.y })))
        }
  ...
  const activeMeasurements = useMemo(() => {
    ...
    return allKeys.map(key => { const v1 = d1Map.get(key), v3 = d3Map.get(key); const avg = v1 != null && v3 != null ? parseFloat(((v1 + v3) / 2).toFixed(4)) : (v1 ?? v3 ?? 0); return { timestamp: new Date(key + ':00:00.000Z').toISOString(), value: avg, unit: sensor?.unit || '', status: 'normal' } })
  ...
  const handleExcelDownload = async () => {
    ...
    if (chartRef.current) {
      try {
        ...
        if (svgEl) {
          const w = svgEl.clientWidth || 800, h = svgEl.clientHeight || 300
          const svgData = new XMLSerializer().serializeToString(svgEl)
          const svgUrl = URL.createObjectURL(new Blob([svgData], { type: 'image/svg+xml;charset=utf-8' }))
          await new Promise<void>(resolve => {
            const img = new Image(); img.width = w; img.height = h
            img.onload = () => { try { const canvas = document.createElement('canvas'); canvas.width = w; canvas.height = h; const ctx = canvas.getContext('2d')!; ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, w, h); ctx.drawImage(img, 0, 0, w, h); const dataUrl = canvas.toDataURL('image/png'); if (dataUrl && dataUrl.startsWith('data:image/png;base64,') && dataUrl.length > 100) chartBase64 = dataUrl } catch { }; URL.revokeObjectURL(svgUrl); resolve() }
            img.onerror = () => { URL.revokeObjectURL(svgUrl); resolve() }; img.src = svgUrl
          })
        }
    ...
    let managerText = '—'
    if (managerUsernames.length > 0) { try { const users = await userApi.getList(); managerText = managerUsernames.map((u: string) => { const f = users.find((x: any) => x.username === u); return f ? `${f.username} (${f.role})` : u }).join(', ') } catch { managerText = managerUsernames.join(', ') } }
    ...
    ws2.mergeCells(legendRow, 1, legendRow, 2); const lgLine = ws2.getCell(legendRow, 1); lgLine.value = '── ' + (sensor.manageNo || sensor.name); lgLine.font = { name: '맑은 고딕', size: 9, color: { argb: 'FF2F5496' } }; lgLine.alignment = { horizontal: 'center', vertical: 'middle' }
  ...
  const handlePdfDownload = async () => {
    ...
    if (chartData.length > 0) {
      ...
      const allVals = [...values, ...refVals]; if (allVals.length === 0) { doc.save(`${sensor.manageNo || sensor.name}_${dateFrom}_${dateTo}.pdf`); return }
      ...
      doc.setFontSize(6); doc.setDrawColor(34, 150, 100); doc.setLineWidth(1.0); doc.line(centerX - 52, legendY, centerX - 44, legendY); doc.setTextColor(34, 150, 100); doc.text(`── ${sensor.manageNo || sensor.name}`, centerX - 42, legendY + 0.5)
    ...
    autoTable(doc, { startY: cy + 75, head: [['측정일', '경과일', `지하수위 G.L(${sensor.unit})`, '전측정대비', '초기치대비', '비고']], body: pdfSortedRows.map((r: any, i: number) => { const rd = new Date(r.timestamp), cm = new Date(rd.getFullYear(), rd.getMonth(), rd.getDate()), im = new Date(pdfInitDate.getFullYear(), pdfInitDate.getMonth(), pdfInitDate.getDate()), el = Math.round((cm.getTime() - im.getTime()) / 86400000), dk = rd.toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }); if (r.value === null) return [dk, '—', '미수신', '—', '—', '']; const cv = parseFloat(parseFloat(String(r.value)).toFixed(4)), prevValid = pdfSortedRows.slice(0, i).reverse().find((x: any) => x.value !== null), pv = prevValid ? parseFloat(parseFloat(String(prevValid.value)).toFixed(4)) : cv, pd = parseFloat((cv - pv).toFixed(4)), id_ = parseFloat((cv - initValue).toFixed(4)), isFirst = pdfSortedRows.findIndex((x: any) => x.value !== null) === i; return [dk, el, cv.toFixed(4), isFirst ? '0.0000' : (pd > 0 ? `+${pd}` : String(pd)), isFirst ? '0.0000' : (id_ > 0 ? `+${id_}` : String(id_)), isFirst ? '초기치' : ''] }), theme: 'grid', headStyles: { fillColor: [60, 80, 120], textColor: 255, fontSize: 8, font: 'NanumGothic', fontStyle: 'normal' }, styles: { fontSize: 8, cellPadding: 2, font: 'NanumGothic' } })
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-surface-page">
      ...
      <div className="shrink-0 border-b border-line bg-surface-card/90 px-4 py-2.5 backdrop-blur-md">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isMultiMonitor && (
              <>
                <button onClick={() => setShowAddSensor(true)} className="flex items-center gap-1 rounded-md border border-brand/30 bg-brand/10 px-3 py-1.5 font-mono text-xs text-brand hover:bg-brand/20">+ 센서 추가</button>
                <button onClick={() => setShowRemoveSensor(true)} className="flex items-center gap-1 rounded-md border border-red-200 bg-red-50 px-3 py-1.5 font-mono text-xs text-red-400 hover:bg-red-100">− 센서 삭제</button>
              </>
            )}
          </div>
        </div>
      ...
      <div className="flex shrink-0" style={{ height: '45vh', minHeight: '260px' }}>
          ...
          <div className="p-4">
            ...
            {sensor ? (
              <dl className="space-y-2">
                {[
                  { l: '관리번호', v: icons.find(i => i.key === currentIconKey)?.label || '' },
                  { l: '센서명', v: sensor.name },
                ...
                {!isMultiMonitor && (
                  <div className="mt-2 rounded-lg border border-line bg-surface-subtle p-2">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {!criteriaEditing ? (
                        <button onClick={() => { setCriteriaEditing(true); if (sensorCode === '80053') { const dc = sensor.criteria?.depthCriteria?.[depthLabel]; setCriteriaInput({ upper: dc?.upper != null ? String(dc.upper) : '', lower: dc?.lower != null ? String(dc.lower) : '' }) } else { setCriteriaInput({ upper: sensor.criteria?.level1Upper ?? '', lower: sensor.criteria?.level1Lower ?? '' }) } }} className="font-mono text-[9px] text-brand hover:underline">수정</button>
                      ) : (
                        <div className="flex gap-1">
                          <button onClick={async () => { setCriteriaSaving(true); try { if (sensorCode === '80053') { const prevDc = sensor.criteria?.depthCriteria || {}; const nextDc = { ...prevDc, [depthLabel]: { upper: criteriaInput.upper !== '' ? parseFloat(criteriaInput.upper) : null, lower: criteriaInput.lower !== '' ? parseFloat(criteriaInput.lower) : null } }; await sensorApi.updateInfo(Number(activeSensorId), { depth_criteria: nextDc }); setSensor((prev: any) => ({ ...prev, criteria: { ...prev.criteria, depthCriteria: nextDc } })) } else { await sensorApi.updateInfo(Number(activeSensorId), { level1_upper: criteriaInput.upper !== '' ? parseFloat(criteriaInput.upper) : null, level1_lower: criteriaInput.lower !== '' ? parseFloat(criteriaInput.lower) : null }); setSensor((prev: any) => ({ ...prev, criteria: { ...prev.criteria, level1Upper: criteriaInput.upper, level1Lower: criteriaInput.lower } })) }; setCriteriaEditing(false) } catch { alert('저장 실패') } finally { setCriteriaSaving(false) } }} disabled={criteriaSaving} className="font-mono text-[9px] text-brand hover:underline disabled:opacity-50">{criteriaSaving ? '저장중' : '저장'}</button>
                    ...
                    {!criteriaEditing ? (
                      <div className="space-y-1">
                        <div className="flex gap-1"><span className="w-16 shrink-0 font-mono text-[10px] text-ink-muted">1차 상한</span><span className="font-mono text-[10px] text-ink">{level1Upper !== null ? `${(level1Upper as number).toFixed(2)} m` : '—'}</span></div>
                        <div className="flex gap-1"><span className="w-16 shrink-0 font-mono text-[10px] text-ink-muted">1차 하한</span><span className="font-mono text-[10px] text-ink">{level1Lower !== null ? `${(level1Lower as number).toFixed(2)} m` : '—'}</span></div>
                      </div>
                    ) : (
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-1"><span className="w-16 shrink-0 font-mono text-[10px] text-ink-muted">1차 상한</span><input type="number" step="0.01" value={criteriaInput.upper} onChange={e => setCriteriaInput(p => ({ ...p, upper: e.target.value }))} className="flex-1 rounded border border-line bg-surface-card px-1.5 py-0.5 font-mono text-[10px] text-ink outline-none focus:border-brand/50" placeholder="예: -3.00" /><span className="font-mono text-[9px] text-ink-muted">m</span></div>
                        <div className="flex items-center gap-1"><span className="w-16 shrink-0 font-mono text-[10px] text-ink-muted">1차 하한</span><input type="number" step="0.01" value={criteriaInput.lower} onChange={e => setCriteriaInput(p => ({ ...p, lower: e.target.value }))} className="flex-1 rounded border border-line bg-surface-card px-1.5 py-0.5 font-mono text-[10px] text-ink outline-none focus:border-brand/50" placeholder="예: -11.00" /><span className="font-mono text-[9px] text-ink-muted">m</span></div>
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <div className="space-y-1">
                      {siteSensors.map((s: any) => (
                        <button key={s.id} onClick={() => setActiveSensorId(s.id)}
                          className={['w-full flex items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors font-mono text-[11px]', activeSensorId === s.id ? 'bg-brand/10 text-brand font-medium' : 'text-ink-sub hover:bg-surface-subtle'].join(' ')}>
              ...
              <div className="space-y-1">
                <p className="font-mono text-[11px] text-ink-muted mb-2">센서를 선택하세요</p>
                {siteSensors.map((s: any) => (
                  <button key={s.id} onClick={() => setActiveSensorId(s.id)}
                    className="w-full flex items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors font-mono text-[11px] text-ink-sub hover:bg-surface-subtle">
                    <span className={['w-2 h-2 rounded-full shrink-0', s.status === 'danger' ? 'bg-sensor-danger' : s.status === 'warning' ? 'bg-sensor-warning' : 'bg-sensor-normal'].join(' ')} />
                    {s.name}
                  </button>
                ))}
        ...
        {/* 리사이즈 핸들 */}
        <div onMouseDown={e => { e.preventDefault(); isResizingLeft.current = true }} className="hidden lg:block w-1 shrink-0 cursor-col-resize bg-transparent hover:bg-brand/30 transition-colors" />
          ...
          <div className="shrink-0 flex items-center justify-between border-b border-line px-3 py-2">
            ...
            {!isMultiMonitor && (
              <div className="flex items-center gap-1">
                ...
                {icons.find(i => i.key === currentIconKey) && (
                  <>
                    <button onClick={() => { const icon = icons.find(i => i.key === currentIconKey); if (icon) { setEditingIcon({ key: icon.key, label: icon.label }); setEditingLabel(icon.label) } }} className="flex items-center gap-1 rounded-md border border-line bg-surface-card px-2.5 py-1 font-mono text-[10px] text-ink-muted hover:border-brand/40 hover:text-brand">✏️ 센서 아이콘 수정</button>
                    <button onClick={() => setShowDeleteIconModal(true)} className="flex items-center gap-1 rounded-md border border-line bg-surface-card px-2.5 py-1 font-mono text-[10px] text-ink-muted hover:border-red-400/40 hover:text-red-400">🗑️ 센서 아이콘 삭제</button>
                  </>
                )}
                <label className="cursor-pointer rounded-md border border-line bg-surface-card px-2 py-1 font-mono text-[10px] text-ink-muted hover:border-brand/40 hover:text-brand">
                  📎 {floorPlanUrl ? '평면도 변경' : '평면도 업로드'}
                  <input type="file" accept="image/jpeg,image/png,image/gif,image/webp,application/pdf" className="hidden"
                    onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; const formData = new FormData(); formData.append('file', file); try { const res = await fetch(`${API_BASE}/api/sites/${site.id}/floor-plan`, { method: 'POST', headers: { Authorization: `Bearer ${localStorage.getItem('gm_token')}` }, body: formData }); const data = await res.json(); if (data.success) { setSite((prev: any) => ({ ...prev, has_floor_plan: true })); setFloorPlanTimestamp(Date.now()) } else alert('업로드 실패: ' + (data.error || '알 수 없는 오류')) } catch { alert('업로드 중 오류가 발생했습니다.') } }} />
                </label>
              </div>
            )}
          </div>
          <div className="flex-1 relative bg-surface-subtle overflow-hidden" ref={floorPlanRef}>
            {floorPlanUrl ? (
              <>
                ...
                {icons.map(icon => (
                  <SensorIcon key={icon.key} icon={icon} isSelected={icon.key === currentIconKey} status={iconStatuses[icon.key] || 'offline'} onMouseDown={e => handleIconMouseDown(icon.key, e)} onClick={() => handleIconClick(icon.key)} />
                ))}
                {icons.length > 0 && (
      ...
      {sensor ? (
        <div className="border-t border-line bg-surface-card flex flex-col shrink-0" style={{ height: '65vh', minHeight: '530px' }}>
          ...
          <div className="shrink-0 flex items-center gap-2 px-3 py-2.5 border-b border-line flex-wrap">
            <div className="flex items-center gap-1">
              {[['오늘', 1], ['7일', 7], ['30일', 30]].map(([label, days]) => (<button key={String(label)} onClick={() => setPreset(Number(days))} className="rounded-md border border-line px-3 py-1 font-mono text-[11px] text-ink-muted hover:bg-surface-subtle hover:text-ink">{label}</button>))}
            </div>
            ...
            <div className="flex gap-1">
              {['시간별', '일별'].map(m => (<button key={m} onClick={() => setChartMode(m === '시간별' ? 'hourly' : 'daily')} className={['rounded-md border px-3 py-1 font-mono text-[11px]', chartMode === (m === '시간별' ? 'hourly' : 'daily') ? 'border-brand/30 bg-brand/10 text-brand' : 'border-line text-ink-muted hover:bg-surface-subtle'].join(' ')}>{m}</button>))}
            </div>
            ...
            {sensorCode === '80053' && (
              <>
                ...
                <div className="flex gap-1">{(isMultiMonitor ? [['Linear', 'linear']] : [['Linear', 'linear'], ['Polynomial', 'poly']]).map(([l, v]) => (<button key={v} onClick={() => setCalcMode(v as 'linear' | 'poly')} className={['rounded-md border px-3 py-1 font-mono text-[11px]', calcMode === v ? 'border-brand/30 bg-brand/10 text-brand' : 'border-line text-ink-muted hover:bg-surface-subtle'].join(' ')}>{l}</button>))}</div>
                <div className="w-px h-4 bg-line shrink-0" />
        ...
        <div className="flex-1 flex items-center justify-center border-t border-line bg-surface-card">
          <div className="text-center"><p className="font-mono text-sm text-ink-muted">센서를 선택하면 데이터가 표시됩니다</p><p className="font-mono text-[11px] text-ink-muted mt-1">평면도의 센서 아이콘 또는 좌측 목록을 클릭하세요</p></div>
        </div>
      )}
